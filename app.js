...
let oppetId = null;
let redigerar = false;
let nyBild = null;   // vald bild som data-URL, an sa lange bara i webblasaren
...
function redigera() {
  ...
  $("dokument").innerHTML = `
    <div class="dok-inre red">
      ...
      <p class="red-hjalp">Har du bakat receptet själv? Byt gärna ut bilden mot ditt eget
        foto. Den nya bilden sparas först när du klickar Spara.</p>
      <div class="red-bild" id="red-bild" tabindex="0" role="button"
           aria-label="Byt bild på receptet">
        ${r.image
          ? `<img id="red-bild-forhands" src="${esc(bildvag(r.image))}" alt="">`
          : `<div id="red-bild-forhands" class="red-bild-tom">🧁</div>`}
        <p class="red-bild-hjalp">Dra hit en bild, klistra in med ⌘V,
          eller klicka för att välja en fil.</p>
      </div>
      <input type="file" id="red-bild-fil" accept="image/jpeg,image/png,image/webp" hidden>
...
// Bildrutan byggs om vid varje redigera(), sa lyssnarna maste sattas har.
// Klicket gar genom den delegerade handlaren langre ner, som alla andra knappar
// i formularet.
function kopplaBildruta() {
  const ruta = $("red-bild");
  $("red-bild-fil").addEventListener("change", (e) => {
    valjBild(e.target.files[0]);
    e.target.value = "";
  });
  ruta.addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); $("red-bild-fil").click(); }
  });
  ruta.addEventListener("dragover", (e) => { e.preventDefault(); ruta.classList.add("drar"); });
  ruta.addEventListener("dragleave", () => ruta.classList.remove("drar"));
  ruta.addEventListener("drop", (e) => {
    e.preventDefault();
    ruta.classList.remove("drar");
    valjBild(e.dataTransfer.files[0]);
  });
}

// Visar bilden direkt men skickar ingenting - den foljer med i PUT:en nar du
// klickar Spara. Avbryter du far den gamla bilden sta kvar, orord pa disken.
function valjBild(fil) {
  if (!fil || !fil.type.startsWith("image/")) {
    return redStatus("Det där var ingen bild. Välj en JPEG, PNG eller WEBP.", "fel");
  }
  if (/heic|heif/i.test(fil.type)) {
    return redStatus("HEIC-bilder går inte att läsa. Spara om fotot som JPEG eller PNG.", "fel");
  }
  if (fil.size > 10 * 1024 * 1024) {
    return redStatus("Bilden är för stor – max 10 MB. Minska den och försök igen.", "fel");
  }
  const lasare = new FileReader();
  lasare.onload = () => {
    nyBild = lasare.result;
    const gammal = $("red-bild-forhands");
    if (gammal.tagName === "IMG") {
      gammal.src = nyBild;
    } else {
      const img = document.createElement("img");
      img.id = "red-bild-forhands";
      img.alt = "";
      img.src = nyBild;
      gammal.replaceWith(img);
    }
    redStatus("Ny bild vald – klicka Spara för att byta ut den gamla.");
  };
  lasare.onerror = () => redStatus("Kunde inte läsa bildfilen.", "fel");
...
async function sparaAndringar() {
  ...
  $("spara").disabled = true;
  redStatus(nyBild ? "Sparar ändringarna och den nya bilden …" : "Sparar …", "jobbar");
  try {
...
// Slapper du bilden bredvid rutan i stallet for i den ska webblasaren inte
// oppna filen - da hade allt osparat i formularet forsvunnit.
for (const typ of ["dragover", "drop"]) {
  document.addEventListener(typ, (e) => { if (redigerar) e.preventDefault(); });
}

// Klistrar du in en bild i redigeringslaget blir den receptets nya bild.
// All annan inklistring - text i falten - lamnas i fred.
document.addEventListener("paste", (e) => {
  if (GAST || !redigerar || !e.clipboardData) return;
  const post = [...e.clipboardData.items].find(
    (i) => i.kind === "file" && i.type.startsWith("image/"));
  if (!post) return;
  e.preventDefault();
  valjBild(post.getAsFile());
});
...
$("dokument").addEventListener("click", (e) => {
  if (e.target.closest("#red-bild")) return $("red-bild-fil").click();
